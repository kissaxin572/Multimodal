...
log_file_path = "Logs/log_20s.txt"  # 定义日志文件路径
...
def process_img(base_folder, output_base_folder):
    ...
    for method_name, method_instance in methods:
        ...
        for benign_count, combined_file in enumerate(tqdm(benign_files, desc=f"处理良性文件 ({method_name})", unit="文件")):
            ...
            # input_file="Datasets/Original/20s/Snapshots/B/B_1/ck1/combined.img",output_file="Datasets/Processed/20s/SFC/Gray/B/1.png"
...
def hpc():
    input_dirs = "Datasets/Original/20s/8Events"
    output_file = "Datasets/Processed/20s/20s_200ms.csv"
...
def b2image():
    input_base_folder = "Datasets/Original/20s/Snapshots"
    output_base_folder = "Datasets/Processed/20s"
